...
def main():
    path = './Results/Yoga5/dense201_hirar/'
    ...
    checkpointer = ModelCheckpoint(filepath=path + 'Variant1.hdf5', verbose=1, save_best_only = True, monitor='val_loss')
    ...
    model = dense201_hirar()
    model.compile(loss=['categorical_crossentropy','categorical_crossentropy','categorical_crossentropy'], loss_weights=[1,1,1], optimizer= sgd, metrics=['accuracy'])#, 'top_k_categorical_accuracy'])
    model.set_weights(np.load(path + 'model_weights_V1.py', allow_pickle=True))
