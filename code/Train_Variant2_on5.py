...
path = '../data2/kaggle_yogaposes/DATASET/TRAIN/'  
...
for r, d, f in os.walk(path):    
    for dic in d:
        path = '../data2/kaggle_yogaposes/DATASET/TRAIN/' + dic + '/'
...
path = '../data2/kaggle_yogaposes/DATASET/TEST/'  
...
for r, d, f in os.walk(path):    
    for dic in d:
        path = '../data2/kaggle_yogaposes/DATASET/TEST/' + dic + '/'
        for r2, d2, f2 in os.walk(path):
            for file in f2:
                data.append((dic, file))
            
test_df = pd.DataFrame(data,columns=['YogaPoses','ImageNumbers'])
test_df['label of class_6']= test_df['YogaPoses'].astype('category').cat.codes
test_df['label of class_20'] = test_df['YogaPoses'].astype('category').cat.codes
test_df['label of class_82'] = test_df['YogaPoses'].astype('category').cat.codes
...
def main():
    path = './Results/Yoga5/dense201_hirar_6same20/'
    ...
    checkpointer = ModelCheckpoint(filepath=path+'Variant2.hdf5', verbose=1, save_best_only = True, monitor='val_loss')
    #Callback that streams epoch results to a CSV file.
    csv_logger= CSVLogger(path+'log_V2.csv')
    ...
    np.save(path + 'model_weights_V2.npy',model.get_weights())
    print('FIT ENDED')
    ...
    model = dense201_hirar_6same20()
    model.compile(loss=['categorical_crossentropy','categorical_crossentropy','categorical_crossentropy'], loss_weights=[1,1,1], optimizer= sgd, metrics=['accuracy'])#, 'top_k_categorical_accuracy'])
    model.set_weights(np.load(path + 'model_weights_V2.npy', allow_pickle=True))
